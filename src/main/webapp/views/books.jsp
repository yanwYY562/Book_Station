...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-bar { display: flex; gap: 20px; margin-bottom: 30px; }
        .filter-group { display: flex; align-items: center; gap: 10px; }
        .filter-label { color: #666; font-size: 14px; }
        .filter-select { padding: 8px 15px; border: 1px solid #ddd; border-radius: 5px; }
        .filter-btn { padding: 8px 20px; background: #667eea; color: white; border: none; border-radius: 5px; cursor: pointer; }
...
<body>
    ...
    <div class="container">
        ...
        <form action="/books" method="get" class="filter-bar">
            <div class="filter-group">
                <span class="filter-label">分类：</span>
                <select name="category" class="filter-select">
                    <option value="" ${selectedCategory == null ? 'selected' : ''}>全部</option>
                    <c:forEach items="${categories}" var="category">
                        <option value="${category.id}" ${selectedCategory != null && selectedCategory == category.id ? 'selected' : ''}>${category.name}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="filter-group">
                ...
                <select name="status" class="filter-select">
                    <option value="" ${selectedStatus == null ? 'selected' : ''}>全部</option>
                    <option value="1" ${selectedStatus != null && selectedStatus == 1 ? 'selected' : ''}>连载中</option>
                    <option value="2" ${selectedStatus != null && selectedStatus == 2 ? 'selected' : ''}>已完结</option>
                </select>
            </div>
            <div class="filter-group">
                <span class="filter-label">类型：</span>
                <select name="free" class="filter-select">
                    <option value="" ${selectedFree == null ? 'selected' : ''}>全部</option>
                    <option value="1" ${selectedFree != null && selectedFree == 1 ? 'selected' : ''}>免费</option>
                    <option value="0" ${selectedFree != null && selectedFree == 0 ? 'selected' : ''}>付费</option>
                </select>
            </div>
