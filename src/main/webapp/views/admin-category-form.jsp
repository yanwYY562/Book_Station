...
<html>
<head>
    <title>${category != null ? '编辑分类' : '添加分类'} - 悦读书城管理后台</title>
    <style>
    ...
    <div class="container">
        ...
        <div class="main-content">
            <div class="page-header">
                <h1 class="page-title">📁 ${category != null ? '编辑分类' : '添加分类'}</h1>
                <a href="/admin/categories" class="back-link">← 返回列表</a>
            </div>

            <div class="form-container">
                <form action="${category != null ? '/admin/categories/update' : '/admin/categories/save'}" method="post">
                    <input type="hidden" name="id" value="${category != null ? category.id : ''}">
                    
                    <div class="form-group">
                        <label>分类名称 *</label>
                        <input type="text" name="name" value="${category != null ? category.name : ''}" required>
                    </div>

                    <div class="form-group">
                        <label>描述</label>
                        <textarea name="description">${category != null ? category.description : ''}</textarea>
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label>排序</label>
                            <input type="number" name="sortOrder" value="${category != null ? category.sortOrder : '0'}">
                        </div>
                        <div class="form-group">
                            <label>状态</label>
                            <select name="status">
                                <option value="1" ${category != null && category.status == 1 ? 'selected' : ''}>启用</option>
                                <option value="0" ${category != null && category.status == 0 ? 'selected' : ''}>禁用</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="/admin/categories" class="cancel-btn">取消</a>
                    </div>
                </form>
            </div>
        </div>
